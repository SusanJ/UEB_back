...
class SubSupIndicator extends Indicator {

 /** Handles simple case of one level indicator at
     a time as the most common or basic case in non-technical
     material.  Does NOT suppory simultanous subscript and
     superscript nor a subscripted or superscripted otem that
...
static boolean subSupIsPending(){
 if (pending == null) return false;
 return true;
}
static String getPendingStartTag(){
...
static String spaceEncountered(){
 if (pending == null) return "";
 ...
 //next line....
static String afterAtEOL(){
 if (pending == null) return "";
 String endTag = pending.getEndTag();
 clearPending();
 return endTag;
}
static String getAfterTag( String nextBrl ){
 System.out.println( "SS getATag pending: "+pending+
 " nextBrl: "+nextBrl );
 if (pending == null) return "";
 String endTag;
